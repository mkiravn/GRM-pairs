...
typedef struct {
    ...
    double   tot_weight; /* running sum of per-locus weights */
                   ...
                   RelPair**     pairs_out)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        pairs[p++] = (RelPair){ idx1, idx2, 0.0, 0.0, 0, 0.0 };
    }
    fclose(f);
...
static double locus_weight(uint32_t snp_idx)
{
    (void)snp_idx;
    return 1.0;
}
...
/* ------------------------------------------------------------------ */
/* Step 4a: precalculate Ms                                           */
/* ------------------------------------------------------------------ */
/*
 * Ms is computed from a second pair list provided by the user.
 * Per-locus statistic:
 *   s_ij = 0.5 * (1 + (X_i - 1)(X_j - 1))
 * where X is the decoded genotype value (0/1/2, missing excluded).
 *
 * Weighted average over loci and pairs:
 *   Ms = sum_k sum_pairs (w_k * s_ij) / sum_k sum_pairs (w_k)
 */
static double calc_Ms(const char*      bed_fname,
                      const RelPair*   ms_pairs,
                      uint32_t         ms_pair_ct,
                      const uint8_t*   needed,
                      uint32_t         sample_ct,
                      uint32_t         snp_ct)
{
    if (ms_pair_ct == 0) {
        fprintf(stderr, "Error: Ms pair list is empty\n");
        return NAN;
    }
    ...
    if (!bed_row || !geno) {
        fprintf(stderr, "Error: out of memory while computing Ms\n");
        free(bed_row);
        free(geno);
        return NAN;
    }
    ...
    for (uint32_t k = 0; k < snp_ct; k++) {
        if (fread(bed_row, 1, row_bytes, bed) != row_bytes) {
            fprintf(stderr, "Error: truncated .bed at SNP %u while computing Ms\n", k);
            fclose(bed);
            free(bed_row);
            free(geno);
            return NAN;
        ...
        double weight = locus_weight(k);
        if (weight <= 0.0) continue;

        decode_bed_row(bed_row, sample_ct, needed, geno);

        for (uint32_t p_idx = 0; p_idx < ms_pair_ct; p_idx++) {
            uint32_t i = ms_pairs[p_idx].idx1;
            uint32_t j = ms_pairs[p_idx].idx2;
            uint8_t xi = geno[i];
            uint8_t xj = geno[j];

            if (xi == MISSING_GENO || xj == MISSING_GENO) continue;

            double sij = 0.5 * (1.0 + ((double)xi - 1.0) * ((double)xj - 1.0));
            ms_num += weight * sij;
            ms_denom += weight;
        }
    }
    ...
    if (ms_denom == 0.0) {
        fprintf(stderr, "Error: no valid loci/pairs available to compute Ms\n");
        return NAN;
    }
    return ms_num / ms_denom;
}
                   ...
                   const char*  out_fname)
{
    ...
    RelPair* ms_pairs = NULL;
    int ms_pair_ct = load_pair_file(ms_pair_fname, sample_ids, sample_ct, &ms_pairs);
    if (ms_pair_ct < 0) {
        free(pairs);
        return -1;
    }
    fprintf(stderr, "Loaded %d Ms pairs\n", ms_pair_ct);

    /* --- precompute needed-sample mask ----------------------------- */
    uint8_t* needed = calloc(sample_ct, 1);
    build_needed_mask(pairs, pair_ct, sample_ct, needed);
    for (int p_idx = 0; p_idx < ms_pair_ct; p_idx++) {
        needed[ms_pairs[p_idx].idx1] = 1;
        needed[ms_pairs[p_idx].idx2] = 1;
    }

    double Ms = calc_Ms(bed_fname, ms_pairs, (uint32_t)ms_pair_ct,
                        needed, sample_ct, snp_ct);
    if (isnan(Ms)) {
        free(pairs);
        free(ms_pairs);
        free(needed);
    ...
    if (fabs(1.0 - Ms) < 1e-12) {
        fprintf(stderr, "Error: Ms is too close to 1.0; estimator undefined\n");
        free(pairs);
        free(ms_pairs);
        free(needed);
        return -1;
    }
    fprintf(stderr, "Computed Ms = %.10g\n", Ms);

    /* --- allocate per-SNP genotype buffer -------------------------- */
    uint8_t* geno = calloc(sample_ct, 1);
    double* x = malloc(sample_ct * sizeof(double));
    if (!geno || !x || !needed) {
        fprintf(stderr, "Error: out of memory in calc_rel_pairs\n");
        free(pairs);
        free(ms_pairs);
        free(needed);
        free(geno);
    ...
    for (uint32_t k = 0; k < snp_ct; k++) {
        ...
        /* precompute genotype values for needed individuals */
        for (uint32_t i = 0; i < sample_ct; i++) {
        ...
        for (uint32_t p_idx = 0; p_idx < (uint32_t)pair_ct; p_idx++) {
            ...
            if (isnan(x[i]) || isnan(x[j])) continue;
            double sij = 0.5 * (1.0 + (x[i] - 1.0) * (x[j] - 1.0));
            pairs[p_idx].numerator += weight * ((sij - Ms) / (1.0 - Ms));
            pairs[p_idx].tot_weight += weight;
    ...
    for (int p_idx = 0; p_idx < pair_ct; p_idx++) {
        if (pairs[p_idx].tot_weight > 0.0) {
            pairs[p_idx].grm = pairs[p_idx].numerator / pairs[p_idx].tot_weight;
        } else {
    ...
    fprintf(out, "IID1\tIID2\tN_SNPs\tCOANC\n");
    for (int p_idx = 0; p_idx < pair_ct; p_idx++) {
