...
write_grm_pheno_multi <- function(df, y_cols, file) {
  missing_cols <- setdiff(y_cols, names(df))
  if (length(missing_cols) > 0) {
    stop(
      paste("Missing phenotype columns:", paste(missing_cols, collapse = ", "))
    )
  }
...
if (!all(pc_names %in% names(pcs))) {
  stop(
    paste(
      "Missing PCs in eigenvec file:",
      paste(setdiff(pc_names, names(pcs)), collapse = ", ")
    )
  )
}
...
dat <- pheno %>%
  ...
  left_join(
    pcs %>% select(participant.eid, all_of(pc_names)),
    by = "participant.eid"
  )
...
f3 <- as.formula(
  paste(
    "phenotype_clean ~ sex + age + yob + batch +",
    paste(pc_names, collapse = " + ")
  )
)
f4 <- as.formula(
  paste(
    "phenotype_clean ~ sex + age + yob + batch + gc +",
    paste(pc_names, collapse = " + ")
  )
)
